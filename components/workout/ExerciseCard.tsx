...
export default function ExerciseCard({
  ...
  const isComplete =
    ...
    sessionExercise.sets.length >= exercise.targetNumberOfSets;
  ...
  const handleAddSet = (setData: Omit<SessionSet, 'completedAt'>) => {
    ...
    <View style={[styles.container, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
      ...
      {isExpanded && (
        <View style={styles.expandedContent}>
          ...
          {/* Set Log Form */}
          <SetLogForm onSubmit={handleAddSet} defaultWeight={defaultWeight} />
          ...
          {sessionExercise && sessionExercise.sets.length > 0 && (
            <View style={styles.setsListContainer}>
              ...
              {sessionExercise.sets.map((set, index) => (
                <SetListItem key={index} set={set} setNumber={index + 1} />
              ))}
            </View>
          )}
        </View>
